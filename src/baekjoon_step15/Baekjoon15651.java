...
public class Baekjoon15651 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        repermutation(arr, result, 0, n, m);

        bw.close();

    }

    private static void repermutation(int[] arr, int[] result, int depth, int n, int m) throws IOException { //중복 순열(자기 자신을 포함하여 뽑는 순열)
        if (depth == m) {
            print(result, m);
            return;
        }
        ...
        for (int i = 0; i < n; i++) { //visit 검사를 위한 for문
            result[depth] = arr[i];
            repermutation(arr, result, depth + 1, n, m);
        }
